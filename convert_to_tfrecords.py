...
tf.app.flags.DEFINE_string('data_dir', './data/',
                           'Directory to Charters (format 1) folders and write the converted files')
...
#        return image
    ...
    def read_and_convert(self, digit_struct_mat_file):
        ...
        digit = int(attrs['length'])
#        image = np.array((Image.open(path_to_image_file)).convert('1').resize([28, 28])).tobytes()
#        print (Image.open(path_to_image_file)).size
        image = np.array((Image.open(path_to_image_file)).resize((28, 28))).tobytes()
#        print (Image.open(path_to_image_file)).convert('1').getdata()[0]
        
        example = tf.train.Example(features=tf.train.Features(feature={
            'image': ExampleReader._bytes_feature(image),
            'digit': ExampleReader._int64_feature(digit),
        }))
        return example
...
def main(_):
    ...
    path_to_val_dir = os.path.join(FLAGS.data_dir, 'val')
    ...
    path_to_val_digit_struct_mat_file = os.path.join(path_to_val_dir, 'digitStruct.mat')
    ...
    [num_val_examples] = convert_to_tfrecords([(path_to_val_dir, path_to_val_digit_struct_mat_file)],
