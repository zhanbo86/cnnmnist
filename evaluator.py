...
#        batch_size = 128
        ...
        with tf.Graph().as_default():
            image_batch,length_batch, digits_batch = Donkey.build_batch_one(path_to_tfrecords_file,
                                                          num_examples=num_examples,
                                                          batch_size=batch_size,
                                                          shuffled=True,num_epoch_=None)
            
            py_x = model(image_batch, w, w2, w3, w4, w_o, p_keep_conv, p_keep_hidden)
            loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=py_x, labels=digits_batch))
